...
import { getQuestionsFromSheet } from "../gsheet";
...
export async function handleStartSurvey(ctx) {
  console.log("handleStartSurvey", ctx);
  const questions = await getQuestionsFromSheet(
    process.env.GOOGLE_SHEET_SURVEY_DATA_ID,
    ctx.from.language_code
  );

  if (ctx.user && ctx.user.questionIndex) {
  ...
  let httpApi = new NetApi(process.env.TELEGRAM_TOKEN_GNSGPTBOT);
  return await httpApi.sendMessage(
    ctx.chat.id,
    `Hi <b>${ctx.from.first_name}!</b> %0AReady to answer a few quick questions and earn $6 SGD of extra money? 💰💰%0A`,
    {
      reply_markup: {
        force_reply: true,
        resize_keyboard: true,
